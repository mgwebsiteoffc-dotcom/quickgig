  ...
  <div class="max-w-shell mx-auto px-5 lg:px-8 grid lg:grid-cols-[1.05fr_0.95fr] gap-14 items-center">
    ...
    <div class="reveal-l">
      ...
      <p class="mt-5 text-[16.5px] leading-7 text-mut max-w-[540px]">
        Quick GIGS writes the brief, matches the right freelancer automatically and holds your money in escrow
        until you approve. Reels, thumbnails, UGC, AI ads and design.
      </p>

      <div class="mt-8 flex flex-wrap items-center gap-3">
        <a href="{{ route('register') }}?type=business" class="h-12 px-6 rounded-xl btn-grad font-semibold text-[14.5px] inline-flex items-center gap-2 shadow-xl shadow-violet/20">
        ...
        <a href="#demo" class="h-12 px-6 rounded-xl glass btn-ghost font-medium text-[14.5px] inline-flex items-center gap-2 hover:border-white/30">
          <span class="w-7 h-7 rounded-full bg-gradient-to-br from-violet to-violet-deep grid place-items-center">
    ...
    <div class="relative reveal-s" x-data="liveOrder()" x-init="start()">
      <div class="absolute -inset-8 bg-gradient-to-br from-violet/22 to-violet/6 blur-3xl rounded-full -z-10"></div>
      ...
      <div class="glass-strong rounded-3xl p-5 sm:p-6 ring-glow">
        ...
        <div class="mt-4 flex items-start gap-3.5">
          <div class="w-14 h-14 rounded-2xl bg-gradient-to-br from-violet to-violet-deep grid place-items-center shrink-0 animate-floaty">
        ...
        <div class="mt-5 flex items-center justify-between glass rounded-2xl px-4 py-3">
          <div class="flex items-center gap-2.5">
            <img src="https://i.pravatar.cc/80?img=5" class="w-8 h-8 rounded-full object-cover border-2 border-violet/35" alt="">
            <div>
...
<section class="py-8 border-y border-white/8 overflow-hidden">
  ...
  <div class="marquee">
    @foreach(array_merge($logos, $logos) as $i => $logo)
      <span class="font-display text-[17px] font-medium whitespace-nowrap {{ ['text-violet-soft/50','text-violet-soft/50','text-amber-soft/50','text-violet-soft/50','text-violet-soft/50'][$i % 5] }}">{{ $logo }}</span>
    @endforeach
  </div>
</section>
...
<section class="band-light py-24">
  <div class="max-w-shell mx-auto px-5 lg:px-8">
    ...
    <div class="mt-12 grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
      @php
        $tones = ['accent' => ['bg-violet/10', 'text-violet-deep', 'from-violet to-violet-deep']];
      @endphp
  ...
  <div class="max-w-shell mx-auto px-5 lg:px-8">
    ...
    <div class="mt-14 grid md:grid-cols-3 gap-5">
      @foreach($steps as $i => $step)
        @php $accent = 'from-violet to-violet-deep'; @endphp
  ...
  <div class="max-w-shell mx-auto px-5 lg:px-8">
    <div class="flex flex-wrap items-end justify-between gap-6 reveal">
      <div class="max-w-[620px]">
        <div class="text-[11px] font-semibold tracking-[.16em] uppercase text-violet-soft">Why this is different</div>
    ...
    <div class="mt-12 grid md:grid-cols-2 lg:grid-cols-3 gap-4">
      @php $uspGrads = ['from-violet to-violet-deep','from-violet to-violet-deep','from-violet to-violet-deep','from-violet to-violet-deep','from-violet to-violet-deep','from-violet to-violet-deep']; @endphp
      @foreach($usps as $i => $u)
...
<section id="demo" class="py-24 border-y border-white/8 relative overflow-hidden">
  <div class="absolute inset-0 -z-10 bg-gradient-to-br from-violet/[0.07] to-transparent"></div>
  <div class="max-w-shell mx-auto px-5 lg:px-8">
    <div class="flex flex-wrap items-end justify-between gap-6 reveal">
      <div class="max-w-[620px]">
        <div class="text-[11px] font-semibold tracking-[.16em] uppercase text-violet-soft">Live simulation</div>
      ...
      <div class="glass rounded-3xl p-6 sm:p-7">
        ...
        <div class="mt-4 grid grid-cols-2 gap-2.5">
          <template x-for="(c, ci) in cfg.categories" :key="c.id">
            <button type="button" x-on:click="pick('category', c.id)"
              ...
              :class="category === c.id ? 'border-violet bg-violet/12 scale-[1.02]' : 'border-white/10 bg-white/3 hover:border-white/30'">
        ...
        <div class="mt-4 space-y-2.5">
          <template x-for="s in cfg.speeds" :key="s.id">
            <button type="button" x-on:click="pick('speed', s.id)"
              ...
              :class="speed === s.id ? 'border-teal bg-violet/12' : 'border-white/10 bg-white/3 hover:border-white/30'">
              <span class="flex items-center gap-3">
                <span class="w-2 h-2 rounded-full transition-colors" :class="speed === s.id ? 'bg-violet' : 'bg-white/20'"></span>
        ...
        <div class="mt-4 flex flex-wrap gap-2">
          <template x-for="a in cfg.addons" :key="a.id">
            <button type="button" x-on:click="toggleAddon(a.id)"
              ...
              :class="addons.includes(a.id) ? 'border-violet bg-violet/12 text-white' : 'border-white/10 text-mut hover:text-white hover:border-white/30'">
        ...
        <div class="mt-7 rounded-2xl border border-white/10 bg-black/25 p-5">
          ...
          <div class="mt-3 space-y-1.5 text-[12.5px] text-mut">
            <div class="flex justify-between"><span>Freelancer receives (90%)</span><span class="font-mono text-white/80" x-text="'₹' + creatorCut.toLocaleString('en-IN')"></span></div>
          ...
          <button type="button" x-on:click="run()" class="mt-5 w-full h-12 rounded-xl btn-grad font-semibold text-[14.5px] shadow-lg shadow-violet/20">
      ...
      <div class="glass-strong rounded-3xl p-6 sm:p-7 flex flex-col">
            ...
            <div class="flex gap-3.5 transition-all duration-500" :class="i <= stage ? 'opacity-100 translate-x-0' : 'opacity-30 translate-x-1'">
              ...
              <div class="pb-2 min-w-0 flex-1">
                ...
                <div x-show="s.key === 'match' && stage >= i" x-cloak x-transition.duration.400ms class="mt-3 flex items-center gap-3 glass rounded-2xl p-3">
                  <img :src="matched.img" class="w-9 h-9 rounded-full object-cover border-2 border-violet/35" alt="">
                  <div class="min-w-0">
        ...
        <div x-show="done" x-cloak x-transition.duration.500ms class="mt-4 rounded-2xl border border-lime/30 bg-lime/10 p-5">
          ...
          <div class="mt-2 text-[14px] leading-6">
            ...
            Escrow released: <span class="font-mono text-lime" x-text="'₹' + creatorCut.toLocaleString('en-IN')"></span> to the freelancer.
          </div>
    ...
    <div class="mt-10 grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
      @foreach($categories as $i => $cat)
        <a href="{{ route('marketplace', ['category' => $cat['q']]) }}"
          ...
          <span class="w-10 h-10 rounded-xl bg-violet/10 text-violet-deep grid place-items-center">
          ...
          <div class="mt-6 flex items-center justify-between">
            ...
            <span class="w-8 h-8 rounded-lg bg-deep/5 text-deep grid place-items-center group-hover:bg-violet group-hover:text-white transition-all duration-300">
...
<section class="py-24">
  <div class="max-w-shell mx-auto px-5 lg:px-8">
    <div class="max-w-[620px] reveal">
      <div class="text-[11px] font-semibold tracking-[.16em] uppercase text-violet-soft">The talent layer</div>
...
<section class="py-20 border-y border-white/8 overflow-hidden">
  <div class="max-w-shell mx-auto px-5 lg:px-8 flex flex-wrap items-end justify-between gap-4 reveal">
    <div>
      <div class="text-[11px] font-semibold tracking-[.16em] uppercase text-violet-soft">Recently delivered</div>
...
<section class="py-24">
  <div class="max-w-shell mx-auto px-5 lg:px-8">
    ...
    <div class="mt-6 grid sm:grid-cols-3 gap-4">
      @foreach([
        ['3 days → 4 hours', 'Average turnaround after Avante Studio moved their reels onto Quick GIGS.', 'from-violet/10 to-transparent'],
        ['−62% revisions', 'Drop in second rounds once briefs were generated instead of written in chat.', 'from-violet/10 to-transparent'],
        ['₹0 wasted', 'Escrow means nothing is paid for work that never gets approved.', 'from-violet/10 to-transparent'],
      ] as $i => [$stat, $body, $grad])
      ...
      @foreach($plans as $i => $p)
        <div class="reveal rounded-3xl p-7 card-hover relative {{ $p['featured'] ? 'glass-strong ring-glow-pink' : 'glass' }}" data-delay="{{ $i * 90 }}">
          ...
          <a href="{{ route('register') }}?type=business&plan={{ $p['slug'] }}"
             class="mt-7 h-12 rounded-xl grid place-items-center font-semibold text-[14px] transition {{ $p['featured'] ? 'btn-grad shadow-lg shadow-violet/20' : 'bg-deep text-white hover:opacity-90' }}">
            {{ $p['cta'] }}
          </a>
        </div>
      @endforeach
  ...
  <div class="max-w-shell mx-auto px-5 lg:px-8">
    <div class="grid md:grid-cols-3 gap-5">
      @foreach($testimonials as $i => $t)
        @php $ring = ['border-violet/35','border-violet/35','border-white/12'][$i % 3]; @endphp
        <figure class="reveal glass rounded-3xl p-7 card-hover" data-delay="{{ $i * 90 }}">
          <div class="text-amber-soft text-[13px] tracking-[.2em]">★★★★★</div>
...
<section class="py-20">
  <div class="max-w-shell mx-auto px-5 lg:px-8">
    <div class="relative overflow-hidden rounded-[32px] px-7 sm:px-12 py-12 bg-gradient-to-br from-violet-deep to-violet">
...
<section class="pb-10">
  <div class="max-w-shell mx-auto px-5 lg:px-8">
    <div class="relative overflow-hidden rounded-[32px] glass-strong px-7 sm:px-14 py-14 sm:py-16 text-center">
      <div class="absolute -top-24 left-1/2 -translate-x-1/2 w-[38rem] h-[38rem] bg-gradient-to-br from-violet/25 to-violet/10 blur-[120px] rounded-full -z-10 animate-drift"></div>
      ...
      <div class="mt-9 flex flex-wrap justify-center gap-3">
        <a href="{{ route('register') }}?type=business" class="px-7 py-3.5 rounded-xl btn-grad font-semibold text-[15px] inline-flex items-center gap-2 shadow-xl shadow-violet/20">
          Start hiring
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4"><path d="M5 12h14M13 6l6 6-6 6"/></svg>
        </a>
        <a href="{{ route('register') }}?type=creator" class="px-7 py-3.5 rounded-xl glass btn-ghost font-medium text-[15px] inline-flex items-center hover:border-white/30">Join as a freelancer</a>
      </div>
