  ...
  <div class="max-w-shell mx-auto px-5 lg:px-8">
    <div class="max-w-[720px]">
      ...
      <div class="mt-8 flex flex-wrap gap-3">
        <a href="{{ route('brief-builder') }}" class="h-12 px-6 rounded-xl btn-grad font-semibold text-[14.5px] inline-flex items-center gap-2 shadow-xl shadow-violet/25">Start with a free brief</a>
        <a href="{{ route('marketplace') }}" class="h-12 px-6 rounded-xl glass font-medium text-[14.5px] inline-flex items-center hover:border-white/30 transition">Skip to the marketplace</a>
      </div>
    </div>
      ...
      @foreach($pipeline as $i => $stage)
        <div class="reveal glass rounded-3xl p-6 sm:p-8 card-hover grid lg:grid-cols-[180px_1fr_200px] gap-6 items-start">
          ...
          <div class="lg:text-right">
            <div class="text-[12px] text-white/70 font-medium">{{ $stage['proof'] }}</div>
            <a href="{{ route($stage['link']) }}" class="mt-3 inline-flex h-10 px-4 rounded-xl glass items-center text-[13px] font-medium hover:border-violet/50 transition">
              See it →
            </a>
          </div>
        </div>
...
<section class="band-light py-24">
  <div class="max-w-shell mx-auto px-5 lg:px-8">
    <div class="grid lg:grid-cols-[0.9fr_1.1fr] gap-12 items-start">
      <div>
        <div class="text-[11px] font-semibold tracking-[.16em] uppercase text-violet-soft">What we guarantee</div>
        ...
        <a href="{{ route('compare') }}" class="mt-7 inline-flex h-11 px-5 rounded-xl glass items-center text-[13.5px] font-medium hover:border-white/30 transition">Compare us honestly →</a>
      </div>
      ...
      <div class="grid sm:grid-cols-2 gap-4">
        @foreach([
          ...
          <div class="reveal glass rounded-3xl p-6" style="transition-delay: {{ $i * 60 }}ms">
            <div class="w-9 h-9 rounded-xl bg-lime/15 grid place-items-center">
