    ...
    <div class="mt-10 grid lg:grid-cols-[1fr_1.15fr] gap-6 items-start">
      <div class="glass rounded-3xl p-6 sm:p-7 space-y-6">
        <template x-for="item in items" :key="item.key">
          <div>
            <div class="flex items-center justify-between text-[13.5px]">
              ...
              <span class="font-mono text-white/80"><span x-text="item.qty"></span> / month</span>
            </div>
        ...
        <label class="flex items-center gap-3 text-[13.5px] text-mut pt-2 border-t border-white/8">
          <input type="checkbox" x-model="retainer" class="w-4 h-4 rounded border-white/20 bg-white/5 accent-violet">
      ...
      <div class="space-y-4">
        <div class="glass-strong rounded-3xl p-6 ring-glow">
          <div class="flex items-baseline justify-between">
            <div>
              <div class="text-[11px] font-semibold tracking-[.14em] uppercase text-white/45">Quick GIGS</div>
          ...
          <div class="mt-4 h-1.5 rounded-full bg-white/8 overflow-hidden"><div class="h-full btn-grad" :style="`width:${barOurs}%`"></div></div>
        </div>
        ...
        <template x-for="alt in alternatives" :key="alt.label">
          <div class="glass rounded-3xl p-6">
            <div class="flex items-baseline justify-between">
              ...
              <div class="font-display text-[24px] font-semibold text-white/70" x-text="'₹' + alt.cost.toLocaleString('en-IN')"></div>
            </div>
            <div class="mt-4 h-1.5 rounded-full bg-white/8 overflow-hidden"><div class="h-full bg-white/25" :style="`width:${alt.bar}%`"></div></div>
          </div>
        </template>

        <div class="rounded-3xl p-6 bg-gradient-to-br from-lime/12 to-transparent border border-lime/20">
          <div class="text-[13px] text-white/75">Versus the cheapest alternative you save</div>
          <div class="mt-1 font-display text-[28px] font-semibold text-lime" x-text="'₹' + saving.toLocaleString('en-IN') + ' / month'"></div>
...
<section class="py-16">
  <div class="max-w-shell mx-auto px-5 lg:px-8">
    <div class="max-w-[620px]">
      <div class="text-[11px] font-semibold tracking-[.16em] uppercase text-violet-soft">Where the money goes</div>
    ...
    <div class="mt-10 grid sm:grid-cols-3 gap-4">
      @foreach([
        ...
        <div class="reveal rounded-3xl p-6 {{ $highlight ? 'glass-strong ring-glow' : 'glass' }}">
          ...
          <div class="mt-4 font-display text-[30px] font-semibold {{ $highlight ? 'text-lime' : 'text-white/70' }}">{{ $amount }}</div>
    ...
    <div class="mt-10 glass rounded-3xl overflow-hidden">
        ...
        <table class="w-full min-w-[820px] text-left">
          <thead>
            <tr class="border-b border-white/10">
              <th class="p-5 text-[11px] font-semibold tracking-[.14em] uppercase text-white/45">How it compares</th>
            ...
            @foreach(array_slice($comparison['rows'], 0, 5) as $row)
              <tr class="border-b border-white/6 last:border-0">
                @foreach($row as $i => $cell)
                  <td class="p-5 align-top text-[13.5px] {{ $i === 0 ? 'text-white/70' : ($i === 1 ? 'text-white font-medium' : 'text-mut') }}">{{ $cell }}</td>
                @endforeach
              </tr>
            @endforeach
          </tbody>
      ...
      <div class="p-5 border-t border-white/8 text-center">
        <a href="{{ route('compare') }}" class="text-[13.5px] text-violet-soft hover:text-white transition">See the full comparison, including where we are not the right choice →</a>
      </div>
    </div>
  </div>
</section>
