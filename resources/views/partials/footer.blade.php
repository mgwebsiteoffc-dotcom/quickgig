<footer class="mt-24 border-t border-white/8">
  <div class="max-w-shell mx-auto px-5 lg:px-8 py-14">
    <div class="grid md:grid-cols-[1.4fr_1fr_1fr_1fr] gap-10">
      <div>
        <a href="{{ route('landing') }}" class="flex items-center gap-2.5">
          <span class="w-9 h-9 rounded-xl grid place-items-center btn-grad">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="#06060B"><path d="M13 2 4 14h7l-1 8 9-12h-7l1-8z"/></svg>
          </span>
          <span class="font-display text-[17px] leading-none"><span class="font-medium text-white/90">Quick</span><span class="font-bold"> GIGS</span></span>
        </a>
        ...
        <div class="mt-5 flex items-center gap-2.5">
          @foreach(['x' => 'M18.9 2H22l-7 8 8.2 12h-6.4l-5-7.3-5.8 7.3H2.9l7.5-9.1L2.2 2h6.6l4.5 6.6z', 'in' => 'M4.98 3.5a2.5 2.5 0 11.02 5 2.5 2.5 0 01-.02-5zM3 9h4v12H3zM10 9h3.8v1.7h.05c.53-1 1.83-2.05 3.77-2.05 4.03 0 4.78 2.6 4.78 6V21h-4v-5.3c0-1.26-.02-2.9-1.8-2.9-1.8 0-2.07 1.38-2.07 2.8V21h-4z'] as $label => $d)
            <a href="#" aria-label="{{ $label }}" class="w-9 h-9 rounded-xl glass grid place-items-center hover:border-violet/50 transition">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" class="text-white/70"><path d="{{ $d }}"/></svg>
            </a>
          @endforeach
        </div>
      </div>
      ...
      @foreach($cols as $head => $links)
        <div>
          <div class="text-[11px] font-semibold tracking-[.14em] uppercase text-white/45">{{ $head }}</div>
    ...
    <div class="mt-12 pt-6 border-t border-white/8 flex flex-col sm:flex-row items-center justify-between gap-3 text-[12.5px] text-white/40">
      <div>© {{ date('Y') }} Quick GIGS. Built in India for the world.</div>
      <div class="flex items-center gap-5">
        <span class="inline-flex items-center gap-1.5"><span class="w-1.5 h-1.5 rounded-full bg-lime"></span> All systems operational</span>
