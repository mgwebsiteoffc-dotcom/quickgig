        ...
        @if($gig->creator)
          <div class="mt-5 glass rounded-3xl p-6">
            <div class="flex items-start gap-4">
              <img src="{{ $gig->creator->avatarUrl() }}" class="w-14 h-14 rounded-2xl object-cover border border-line" alt="">
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2">
                  <a href="{{ route('creator.public', $gig->creator->id) }}" class="font-display text-[17px] font-semibold hover:text-mint-deep transition">{{ $gig->creator->name }}</a>
                ...
                <div class="mt-4 flex flex-wrap gap-2">
                  @foreach((array) ($gig->creator->skills ?? []) as $skill)
                    <span class="text-[11.5px] rounded-full border border-line px-2.5 py-1 text-mut">{{ $skill }}</span>
                  @endforeach
                </div>
              </div>
            </div>
      ...
      <aside class="lg:sticky lg:top-24" x-data="checkout({{ (int) $gig->price }})">
        <div class="glass-strong rounded-3xl p-6 ring-glow">
          ...
          <div class="mt-3 space-y-2">
            @foreach(\App\Http\Controllers\OrderController::LANES as $key => $lane)
              <label class="flex items-center justify-between gap-3 rounded-2xl px-4 py-3 border cursor-pointer transition"
                     :class="lane === '{{ $key }}' ? 'border-mint bg-mint-wash' : 'border-line bg-tint hover:border-line'">
          ...
          <div class="mt-5 rounded-2xl border border-line bg-tint p-4 space-y-2 text-[12.5px]">
            ...
            <div class="flex justify-between"><span class="text-mut">Freelancer receives</span><span class="font-mono text-mint-deep" x-text="'₹' + payout.toLocaleString('en-IN')"></span></div>
