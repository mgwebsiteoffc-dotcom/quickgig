...
<div class="w-full max-w-[420px] bg-white border border-[#E8E8E6] rounded-2xl p-6 sm:p-7 shadow-sm">
  ...
  <form method="POST" action="{{ route('login.post') }}" class="mt-5 space-y-3">
    ...
    <div>
      ...
      <input name="email" type="email" value="{{ old('email') }}" autocomplete="username" required class="mt-1 w-full h-11 px-3 rounded-xl border border-[#E8E8E6] bg-[#F8F8F7] text-[14px] font-medium outline-none focus:bg-white focus:border-[#0F0F0F]">
    </div>
    <div>
      <label class="text-[11px] font-bold tracking-widest uppercase text-[#7A7A78]">Password</label>
      <input name="password" type="password" autocomplete="current-password" required class="mt-1 w-full h-11 px-3 rounded-xl border border-[#E8E8E6] bg-[#F8F8F7] text-[14px] font-medium outline-none focus:bg-white focus:border-[#0F0F0F]">
    </div>
  ...
  <div class="mt-4 text-center">
    <a href="{{ route('password.request') }}" class="text-[12px] font-bold text-[#7A7A78] underline">Forgot your password?</a>
  </div>
