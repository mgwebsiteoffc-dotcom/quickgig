...
<?php echo $__env->make('components.seo', ['seo'=>$seo, 'faqs'=>$faqs], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
...
<header class="sticky top-0 z-40 bg-white/90 backdrop-blur border-b border-line">
  <div class="max-w-[1160px] mx-auto px-4 sm:px-6 h-[64px] flex items-center justify-between gap-4">
    ...
    <nav class="hidden md:flex items-center gap-6 text-[13px] font-semibold text-muted">
      <a href="<?php echo e(route('onboarding.business')); ?>" class="hover:text-ink">Business onboarding</a>
      <a href="<?php echo e(route('onboarding.creator')); ?>" class="hover:text-ink">Creator onboarding</a>
    ...
    <div class="flex items-center gap-2">
      <a href="<?php echo e(route('login')); ?>" class="hidden sm:inline-flex h-9 px-4 rounded-full border border-line font-bold text-[13px] items-center hover:bg-bg">Login</a>
      <a href="<?php echo e(route('onboarding.business')); ?>" class="inline-flex h-9 px-5 rounded-full bg-brand text-white font-bold text-[13px] items-center gap-1.5 hover:bg-blue-700">For Business <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg></a>
    </div>
  </div>
</header>
  ...
  <div class="max-w-[1160px] mx-auto px-4 sm:px-6 py-8 sm:py-12 grid lg:grid-cols-[1.1fr_0.9fr] gap-8 items-center">
    <div>
      ...
      <div class="mt-6 flex flex-wrap gap-3">
        <a href="<?php echo e(route('onboarding.business')); ?>" class="h-[46px] px-7 rounded-full bg-ink text-white font-extrabold text-[14px] inline-flex items-center gap-2 hover:bg-black">I need content — Start in 45 sec <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M5 12h14M12 5l7 7-7 7"/></svg></a>
        <a href="<?php echo e(route('onboarding.creator')); ?>" class="h-[46px] px-6 rounded-full bg-white border border-line font-bold text-[14px] inline-flex items-center gap-2 hover:bg-bg">I’m a Creator <span class="text-[11px] bg-green-50 text-green-700 border border-green-200 px-2 py-0.5 rounded-full">Earn 90% • Weekly UPI</span></a>
      </div>
...
<section class="border-b border-line bg-white">
  <div class="max-w-[1160px] mx-auto px-4 sm:px-6 py-4 flex flex-wrap items-center justify-between gap-3 text-[12px] font-extrabold tracking-widest uppercase text-muted">
    ...
    <div class="flex flex-wrap gap-2">
      <?php $__currentLoopData = $logos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $logo): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><span class="px-3 py-1.5 bg-bg border border-line rounded-full text-ink normal-case tracking-normal font-bold text-[12px]"><?php echo e($logo); ?></span><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </div>
  </div>
</section>
...
<section class="max-w-[1160px] mx-auto px-4 sm:px-6 py-10">
  ...
  <div class="mt-8 grid lg:grid-cols-[1.1fr_0.9fr] gap-6">
    ...
    <div class="bg-ink text-white rounded-2xl p-6">
      ...
      <a href="<?php echo e(route('business.home')); ?>" class="mt-5 inline-flex h-10 px-6 rounded-full bg-white text-ink font-extrabold text-[13px] items-center">Try Instant Assign — ₹1,299</a>
      ...
      <div class="bg-white border border-line rounded-2xl p-5 flex flex-col">
        <div class="flex items-center justify-between">
          <div class="w-9 h-9 rounded-xl bg-ink text-white grid place-items-center">
            <?php if($p['icon']=='zap'): ?><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>
            <?php elseif($p['icon']=='users'): ?><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
            <?php else: ?><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 21V9"/></svg><?php endif; ?>
          </div>
          <span class="text-[11px] font-bold bg-amber-400 text-ink px-2.5 py-1 rounded-full"><?php echo e($p['badge']); ?></span>
        </div>
        <div class="mt-3 text-[16px] font-black"><?php echo e($p['title']); ?></div>
        <div class="text-[12px] font-bold text-muted"><?php echo e($p['time']); ?> • <?php echo e($p['price']); ?></div>
        <div class="mt-2 text-[13px] leading-5 text-muted font-medium flex-1"><?php echo e($p['desc']); ?></div>
        <div class="mt-3 text-[11px] font-bold bg-bg border border-line rounded-full px-3 py-1.5 inline-flex self-start">Best for: <?php echo e($p['for']); ?></div>
        <a href="<?php echo e(route('business.home')); ?>" class="mt-4 h-9 rounded-full bg-ink text-white font-bold text-[13px] grid place-items-center hover:bg-black">Try <?php echo e($p['title']); ?></a>
      </div>
...
<section class="max-w-[1160px] mx-auto px-4 sm:px-6 py-10">
  <div class="flex items-end justify-between gap-4">
    ...
    <a href="<?php echo e(route('business.home')); ?>" class="hidden sm:inline-flex text-[12px] font-bold text-muted hover:text-ink">Browse all services →</a>
  </div>
  <div class="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4">
    <?php $__currentLoopData = $services; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    <a href="<?php echo e(route('business.home')); ?>" class="group bg-white border border-line rounded-2xl overflow-hidden hover:shadow-md transition">
      <div class="relative"><img src="<?php echo e($s['img']); ?>" class="h-[130px] w-full object-cover"><span class="absolute left-2 top-2 text-[11px] font-bold bg-white border border-line px-2 py-1 rounded-full"><?php echo e($s['badge']); ?></span><span class="absolute right-2 bottom-2 text-[11px] font-bold bg-ink text-white px-2 py-1 rounded-full"><?php echo e($s['time']); ?></span></div>
      <div class="p-3"><div class="text-[13px] font-bold leading-tight group-hover:text-brand"><?php echo e($s['title']); ?></div><div class="mt-1 flex items-center justify-between"><span class="text-[13px] font-black"><?php echo e($s['price']); ?></span><span class="text-[11px] font-semibold text-muted"><?php echo e($s['orders']); ?> orders • 4.9★</span></div></div>
    </a>
    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
  </div>
</section>
    ...
    <div class="mt-6 grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div class="bg-white border border-line rounded-2xl p-4">
        <div class="flex gap-3">
          <div class="relative shrink-0"><img src="<?php echo e($c['img']); ?>" class="w-12 h-12 rounded-full object-cover border border-line"><span class="absolute -bottom-1 -right-1 w-[18px] h-[18px] rounded-full bg-[#1D9BF0] text-white grid place-items-center border-2 border-white" style="aspect-ratio:1/1"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="3"><path d="M20 6L9 17l-5-5"/></svg></span></div>
          <div class="min-w-0"><div class="text-[13px] font-black leading-tight flex items-center gap-1"><?php echo e($c['name']); ?> <span class="w-2 h-2 rounded-full <?php echo e($c['available'] ? 'bg-green-500' : 'bg-amber-400'); ?>"></span></div><div class="text-[11px] font-semibold text-muted truncate"><?php echo e($c['handle']); ?></div><div class="text-[11px] font-medium text-muted truncate"><?php echo e($c['role']); ?></div></div>
        </div>
        <div class="mt-3 flex items-center justify-between"><span class="text-[11px] font-bold px-2 py-1 rounded-full border <?php echo e($c['available'] ? 'bg-green-50 border-green-200 text-green-700' : 'bg-amber-50 border-amber-200 text-amber-700'); ?>"><?php echo e($c['available'] ? 'Available now' : 'In 2 hours'); ?></span><span class="text-[12px] font-black"><?php echo e($c['price']); ?></span></div>
        <a href="<?php echo e(route('business.home')); ?>" class="mt-3 h-8 rounded-full bg-ink text-white font-bold text-[12px] grid place-items-center">View portfolio</a>
      </div>
      <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </div>
  </div>
...
<section id="pricing" class="max-w-[1160px] mx-auto px-4 sm:px-6 py-10">
  ...
  <div class="mt-6 grid md:grid-cols-3 gap-4">
    <div class="bg-white border border-line rounded-2xl p-6">
      ...
      <a href="<?php echo e(route('business.home')); ?>" class="mt-5 h-10 rounded-full border-2 border-ink font-extrabold text-[13px] grid place-items-center">Start at ₹1,299</a>
    </div>
    <div class="bg-ink text-white rounded-2xl p-6 border-2 border-ink relative">
      ...
      <a href="<?php echo e(route('business.home')); ?>" class="mt-5 h-10 rounded-full bg-white text-ink font-extrabold text-[13px] grid place-items-center">Get my reel tomorrow</a>
    </div>
    <div class="bg-white border border-line rounded-2xl p-6">
      ...
      <a href="<?php echo e(route('business.home')); ?>" class="mt-5 h-10 rounded-full border-2 border-ink font-extrabold text-[13px] grid place-items-center">Build my team pack</a>
    </div>
  </div>
</section>
  ...
  <div class="max-w-[1160px] mx-auto px-4 sm:px-6 py-10">
    ...
    <div class="mt-6 grid md:grid-cols-3 gap-4">
      ...
      <div class="bg-white border border-line rounded-2xl p-5">
        ...
        <div class="mt-2 text-[13px] leading-6 font-medium">“<?php echo e($t['text']); ?>”</div>
        <div class="mt-4 flex items-center gap-3"><div class="w-9 h-9 rounded-full bg-ink text-white grid place-items-center font-black text-[12px]"><?php echo e(substr($t['name'],0,1)); ?></div><div><div class="text-[13px] font-bold"><?php echo e($t['name']); ?></div><div class="text-[11px] font-semibold text-muted"><?php echo e($t['company']); ?></div></div></div>
      </div>
      <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </div>
  </div>
...
<section id="faq" class="max-w-[760px] mx-auto px-4 sm:px-6 py-10" x-data="{open:0}">
  ...
  <div class="mt-6 divide-y divide-line border border-line rounded-2xl bg-white overflow-hidden">
    ...
    <?php $q = $f->question ?? $f['question'] ?? $f->q ?? ''; $a = $f->answer ?? $f['answer'] ?? $f->a ?? ''; ?>
    <div class="cursor-pointer" @click="open = open === <?php echo e($i); ?> ? -1 : <?php echo e($i); ?>">
      <div class="flex items-center justify-between gap-4 p-4">
        <div class="text-[13px] font-bold leading-tight"><?php echo e($q); ?></div>
        <div class="w-7 h-7 rounded-full border border-line grid place-items-center shrink-0" :class="open === <?php echo e($i); ?> ? 'bg-ink text-white border-ink' : 'bg-white'"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 9l6 6 6-6"/></svg></div>
      </div>
      <div x-show="open === <?php echo e($i); ?>" x-collapse class="px-4 pb-4 text-[13px] leading-6 text-muted font-medium"><?php echo e($a); ?></div>
    </div>
    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
  </div>
  <div class="mt-3 text-center"><a href="<?php echo e(route('blog.index')); ?>" class="text-[12px] font-bold text-brand hover:underline">Browse all FAQs + Playbooks →</a></div>
</section>
...
<section class="bg-bg border-y border-line">
  <div class="max-w-[1160px] mx-auto px-4 sm:px-6 py-10">
    <div class="flex flex-wrap items-end justify-between gap-3">
      ...
      <a href="<?php echo e(route('blog.index')); ?>" class="h-9 px-4 rounded-full border border-line bg-white font-bold text-[13px] inline-flex items-center">View all blogs →</a>
    </div>
    <div class="mt-6 grid md:grid-cols-3 gap-4">
      ...
      <a href="<?php echo e(route('blog.show', $b->slug)); ?>" class="bg-white border border-line rounded-2xl overflow-hidden hover:shadow-md transition group">
        <img src="<?php echo e(filter_var($b->cover, FILTER_VALIDATE_URL) ? $b->cover : asset('storage/'.$b->cover)); ?>" alt="<?php echo e($b->cover_alt ?: $b->title); ?>" class="h-[160px] w-full object-cover">
        <div class="p-4"><div class="text-[11px] font-bold tracking-widest uppercase" style="color:<?php echo e($b->category->color ?? '#2563EB'); ?>"><?php echo e($b->category->name ?? 'Playbook'); ?> • <?php echo e($b->reading_minutes); ?> min</div><div class="mt-1 text-[14px] font-black leading-tight group-hover:text-brand line-clamp-2"><?php echo e($b->title); ?></div><div class="mt-1 text-[12px] leading-5 text-muted line-clamp-2"><?php echo e($b->excerpt); ?></div></div>
      </a>
      <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </div>
  </div>
...
<section class="bg-ink text-white">
  <div class="max-w-[1160px] mx-auto px-4 sm:px-6 py-10 flex flex-col md:flex-row items-center justify-between gap-6">
    ...
    <div class="flex gap-3 shrink-0">
      <a href="<?php echo e(route('onboarding.business')); ?>" class="h-11 px-7 rounded-full bg-white text-ink font-extrabold text-[14px] inline-flex items-center">Start as Business</a>
      <a href="<?php echo e(route('onboarding.creator')); ?>" class="h-11 px-7 rounded-full bg-white/10 border border-white/20 text-white font-bold text-[14px] inline-flex items-center">Join as Creator</a>
    </div>
  </div>
</section>
...
<footer class="bg-white border-t border-line">
  <div class="max-w-[1160px] mx-auto px-4 sm:px-6 py-6 flex flex-col md:flex-row items-center justify-between gap-3 text-[12px] font-medium text-muted">
    <div>© <?php echo e(date('Y')); ?> QuickContent — India’s First Quick Content Delivery Platform. Built for Hostinger shared hosting (no Redis).</div>
    <div class="flex flex-wrap gap-4 font-bold"><a href="<?php echo e(route('onboarding.business')); ?>" class="hover:text-ink">Get Started</a><a href="<?php echo e(route('business.home')); ?>" class="hover:text-ink">Marketplace</a><a href="<?php echo e(route('blog.index')); ?>" class="hover:text-ink">Blog</a><a href="<?php echo e(route('creator.dashboard')); ?>" class="hover:text-ink">Creator</a><a href="<?php echo e(route('admin.dashboard')); ?>" class="hover:text-ink">Admin</a><a href="<?php echo e(route('sitemap')); ?>" class="hover:text-ink">Sitemap</a><a href="/health" class="hover:text-ink">Health</a></div>
  </div>
...
<?php /**PATH C:\laragon\www\quick-content-laravel\resources\views/landing.blade.php ENDPATH**/ ?>