...
<?php echo $__env->make('components.seo', ['seo'=>$seo], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
  ...
  <div class="flex-1 bg-[#F8F8F7] flex flex-col" x-data="{ step: 1, cat: 'Reel', logoPreview: '' }">
    <div class="h-[56px] border-b border-[#E8E8E6] bg-white flex items-center justify-between px-6">
      <div class="flex items-center gap-2 text-[12px] font-bold">
        <a href="<?php echo e(route('landing')); ?>" class="text-[#6B7280] hover:text-ink">Home</a><span class="text-[#E8E8E6]">/</span><span class="text-ink">Business onboarding</span>
      </div>
    ...
    <form method="POST" action="<?php echo e(route('onboarding.business.store')); ?>" enctype="multipart/form-data" class="flex-1 px-6 sm:px-10 py-6 flex flex-col max-w-[560px] w-full mx-auto">
      <?php echo csrf_field(); ?>
      <!-- STEP 1 -->
      <div x-show="step===1" x-transition class="space-y-4">
        <div class="bg-white border border-[#E8E8E6] rounded-2xl p-5">
          ...
          <div class="mt-4 grid gap-3">
            <div><label class="text-[11px] font-bold tracking-widest uppercase text-muted">Company Name *</label><input name="company_name" required placeholder="Avante Studio" value="<?php echo e(old('company_name')); ?>" class="mt-1 w-full h-11 px-3 rounded-xl border border-line bg-bg text-[14px] font-semibold focus:bg-white outline-none"></div>
            <div class="grid grid-cols-2 gap-3">
              <div><label class="text-[11px] font-bold tracking-widest uppercase text-muted">Your Name *</label><input name="person_name" required placeholder="Rohan Sharma" value="<?php echo e(old('person_name')); ?>" class="mt-1 w-full h-11 px-3 rounded-xl border border-line bg-bg text-[14px]"></div>
              <div><label class="text-[11px] font-bold tracking-widest uppercase text-muted">Phone *</label><input name="phone" required placeholder="98765 43210" value="<?php echo e(old('phone')); ?>" class="mt-1 w-full h-11 px-3 rounded-xl border border-line bg-bg text-[14px]"></div>
            </div>
            <div><label class="text-[11px] font-bold tracking-widest uppercase text-muted">Work Email *</label><input name="email" type="email" required placeholder="rohan@avante.studio" value="<?php echo e(old('email')); ?>" class="mt-1 w-full h-11 px-3 rounded-xl border border-line bg-bg text-[14px]"></div>
            <div class="grid grid-cols-2 gap-3">
              <div><label class="text-[11px] font-bold tracking-widest uppercase text-muted">Industry</label><input name="industry" placeholder="D2C, Agency, SaaS" value="<?php echo e(old('industry')); ?>" class="mt-1 w-full h-11 px-3 rounded-xl border border-line bg-bg text-[13px]"></div>
              <div><label class="text-[11px] font-bold tracking-widest uppercase text-muted">Team Size</label><input name="team_size" type="number" placeholder="14" value="<?php echo e(old('team_size')); ?>" class="mt-1 w-full h-11 px-3 rounded-xl border border-line bg-bg text-[13px]"></div>
            </div>
            <div>
      ...
      <?php if($errors->any()): ?><div class="mt-4 bg-red-50 border border-red-200 text-red-700 rounded-xl px-3 py-2 text-[13px] font-semibold"><?php echo e($errors->first()); ?></div><?php endif; ?>
      <div class="mt-6 text-center text-[11px] font-semibold text-muted">Already have an account? <a href="<?php echo e(route('business.home')); ?>" class="text-ink underline">Go to marketplace</a> • <a href="<?php echo e(route('login')); ?>" class="text-ink underline">Login</a></div>
    </form>
  </div>
</div>
</body>
</html>
<?php /**PATH C:\laragon\www\quick-content-laravel\resources\views/onboarding/business.blade.php ENDPATH**/ ?>