  ...
  <div class="creator-row">
    ...
    <div class="creator-card">
      <div class="creator-top">
        <div class="creator-av"><img src="<?php echo e($c['img']); ?>" alt=""><span class="creator-check"><svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12l5 5l10-10"/></svg></span></div>
        <div><div class="creator-name"><?php echo e($c['name']); ?></div><div class="creator-handle"><?php echo e($c['handle']); ?></div></div>
      </div>
      <div class="creator-stats"><div class="creator-stat"><div class="creator-stat-num"><?php echo e($c['followers']); ?></div><div class="creator-stat-label">Followers</div></div><div class="creator-stat"><div class="creator-stat-num"><?php echo e($c['posts']); ?></div><div class="creator-stat-label">Posts</div></div></div>
    </div>
...
<div class="section">
  ...
  <div class="creator-row">
    ...
    <div style="min-width:92px;background:var(--card);border:1px solid var(--line);border-radius:14px;padding:10px;text-align:center;cursor:pointer;flex-shrink:0" onclick="openChoosePro()">
      <div style="width:44px;height:44px;border-radius:50%;margin:0 auto;overflow:hidden;position:relative;border:1px solid var(--line)"><img src="<?php echo e($p['img']); ?>" style="width:100%;height:100%;object-fit:cover"><span style="position:absolute;bottom:0;right:0;width:11px;height:11px;border-radius:50%;border:2px solid white;background:<?php echo e($p['avail']=='on' ? 'var(--green)' : '#EAB308'); ?>;display:block"></span></div>
      <div style="font-size:11.5px;font-weight:700;margin-top:6px"><?php echo e(explode(' ', $p['name'])[0]); ?></div><div style="font-size:10px;color:var(--muted)"><?php echo e($p['handle']); ?></div><div style="font-size:10px;font-weight:700;margin-top:4px;background:<?php echo e($p['avail']=='on' ? 'var(--green-bg)' : 'var(--bg)'); ?>;color:<?php echo e($p['avail']=='on' ? 'var(--green)' : 'var(--muted)'); ?>;padding:2px 6px;border-radius:20px;display:inline-block;border:1px solid <?php echo e($p['avail']=='on' ? 'var(--green-line)' : 'var(--line)'); ?>"><?php echo e($p['label']); ?></div>
    </div>
    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
  </div>
  <div class="banner" onclick="handlePrompt('3 reels + thumbnails for <?php echo e($current['company']); ?> launch')">
    <div class="banner-left"><div class="banner-icon"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="1.8"><path d="M12 3l2 5 5 2-5 2-2 5-2-5-5-2 5-2 2-5z"/></svg></div><div><div class="banner-title">Prompt a team for <?php echo e($current['company']); ?></div><div class="banner-desc">Describe outcome → get team & fixed price in seconds</div></div></div>
...
<div class="section" id="reorderWrap" style="display:none">
  <div class="section-head"><div class="section-title"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M21 12a9 9 0 1 1-2.6-6.4"/><path d="M21 3v7h-7"/></svg> Reorder for <?php echo e($current['company']); ?></div></div>
  <div class="reorder"><div class="reorder-img"><img id="reorderImg" src="https://images.unsplash.com/photo-1574717025058-2f8737d2e2b7?w=200&q=80"></div><div><div class="reorder-title" id="reorderTitle">Engaging Talking-Head Reel</div><div class="reorder-sub" id="reorderSub">Last for <?php echo e($current['company']); ?> • Priya S. • ★5.0</div></div><div class="reorder-cta">Reorder</div></div>
</div>
...
<div class="section" id="bestSection">
  ...
  <div class="grid" id="serviceGrid">
    ...
    <div class="card" data-category="<?php echo e($s['category'] ?? 'Reel'); ?>" data-price-type="<?php echo e($s['price_type'] ?? 'paid'); ?>" onclick="openSheet('<?php echo e($s['id']); ?>')">
      <div class="card-img"><img src="<?php echo e($s['img']); ?>" loading="lazy"><span class="card-badge" style="<?php echo e(($s['price_type'] ?? '')=='barter' ? 'background:#FFC300;color:#0F0F0F' : (($s['category']??'')=='UGC Video' ? 'background:var(--blue);color:white' : '')); ?>"><?php echo e($s['badge'] ?? ($s['category'] ?? 'Best seller')); ?></span><span class="card-play"><svg width="12" height="12" viewBox="0 0 24 24" fill="var(--text)"><path d="M8 5v14l11-7z"/></svg></span></div>
      <div class="card-body">
        <div class="card-title"><?php echo e($s['title']); ?></div>
        <div class="text-[11px] font-semibold" style="color:var(--muted)"><?php echo e($s['category'] ?? ''); ?> <?php if(isset($s['price_type']) && $s['price_type']=='barter'): ?> • <span style="background:#FFF7CC;border:1px solid #FFE27A;padding:1px 6px;border-radius:20px;font-size:10px;font-weight:800;color:#0F0F0F">Barter</span> <?php elseif(isset($s['price_type']) && $s['price_type']=='hybrid'): ?> • Hybrid <?php endif; ?></div>
        <div class="card-creator"><img src="<?php echo e($s['cimg']); ?>"><span><?php echo e($s['creator']); ?> • <?php echo e($s['handle']); ?></span><span class="tick"><svg width="7" height="7" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12l5 5l10-10"/></svg></span></div>
        <div class="card-bottom"><span class="card-price"><?php echo e($s['display_price'] ?? (isset($s['price']) ? '₹'.number_format($s['price']) : '—')); ?></span><span class="card-rating"><svg width="11" height="11" viewBox="0 0 24 24" fill="var(--text)"><path d="M12 2l2.4 7.2H22l-6.2 4.5 2.4 7.2L12 16 5.8 20.9 8.2 13.7 2 9.2h7.6z"/></svg> <?php echo e($s['rating']); ?></span></div>
        <div class="card-meta"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 3"/></svg> <?php echo e($s['time']); ?> <span>•</span> <?php echo e($s['sold']); ?> sold</div>
        <button class="card-add" onclick="event.stopPropagation();openSheet('<?php echo e($s['id']); ?>')"><?php if(($s['price_type']??'paid')=='barter'): ?><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2"><path d="M12 5v14M5 12h14"/></svg> Collab for <?php echo e(explode(' ', $current['company'])[0]); ?> <?php else: ?><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2"><path d="M12 5v14M5 12h14"/></svg> Add for <?php echo e(explode(' ', $current['company'])[0]); ?> <?php endif; ?></button>
      </div>
    </div>
    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
  </div>
...
<div class="section" style="padding-bottom:20px">
  ...
  <div class="grid">
    <?php $ugcServices = array_filter($services, fn($s)=>($s['category']??'')=='UGC Video' || ($s['category']??'')=='Barter Collab' || ($s['price_type']??'')=='barter'); if(empty($ugcServices)) $ugcServices = array_slice($services,0,3); ?>
    <?php $__currentLoopData = array_slice($ugcServices,0,3); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    <div class="card" data-category="<?php echo e($s['category'] ?? ''); ?>" onclick="openSheet('<?php echo e($s['id']); ?>')">
      <div class="card-img"><img src="<?php echo e($s['img']); ?>"><span class="card-badge" style="<?php echo e(($s['price_type']??'')=='barter'?'background:#FFC300;color:#0F0F0F':'background:var(--blue);color:white'); ?>"><?php echo e($s['badge'] ?? (($s['category']??'')=='Barter Collab' ? 'Barter' : 'UGC')); ?></span><span class="card-play"><svg width="12" height="12" viewBox="0 0 24 24" fill="var(--text)"><path d="M8 5v14l11-7z"/></svg></span></div>
      <div class="card-body">
        <div class="card-title"><?php echo e($s['title']); ?></div>
        <div class="card-creator"><img src="<?php echo e($s['cimg']); ?>"><span><?php echo e($s['creator']); ?> • <?php echo e($s['handle']); ?></span><span class="tick"><svg width="7" height="7" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="3"><path d="M5 12l5 5l10-10"/></svg></span></div>
        <div class="card-bottom"><span class="card-price"><?php echo e($s['display_price'] ?? (isset($s['price'])?'₹'.number_format($s['price']):'—')); ?></span><span class="card-rating"><svg width="11" height="11" viewBox="0 0 24 24" fill="var(--text)"><path d="M12 2l2.4 7.2H22l-6.2 4.5 2.4 7.2L12 16 5.8 20.9 8.2 13.7 2 9.2h7.6z"/></svg> <?php echo e($s['rating']); ?></span></div>
        <div class="card-meta"><?php echo e($s['category'] ?? 'UGC'); ?> <span>•</span> <?php echo e($s['time']); ?> <span>•</span> <?php echo e($s['sold']); ?> sold</div>
        <button class="card-add" onclick="event.stopPropagation();openSheet('<?php echo e($s['id']); ?>')"><?php if(($s['price_type']??'')=='barter'): ?> Collab free — ship product <?php else: ?> Add for <?php echo e(explode(' ', $current['company'])[0]); ?> <?php endif; ?></button>
      </div>
    </div>
    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
  </div>
...
const current = <?php echo json_encode($current, 15, 512) ?>;
let sel = services[0];
function openSheet(id){
  ...
  document.getElementById('sheetContent').innerHTML = `
    ...
    <div style="padding:16px">
      ...
      <form method="POST" action="<?php echo e(route('orders.store')); ?>" style="margin-top:16px">
        <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\quick-content-laravel\resources\views/business/home.blade.php ENDPATH**/ ?>